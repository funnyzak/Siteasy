        ...
        public new static string ConfigFilePath
        {
            get
            {
                if (filename == null)
                {
                    ...
                    if (context != null)
                    {
                        filename = context.Server.MapPath("~/STA.config");
                    ...
                    {
                        filename = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "STA.config");
                    }

                    if (!File.Exists(filename))
                    {
                        throw new STAException("发生错误: 虚拟目录或网站根目录下没有正确的STA.config文件");
                    }
                }
                return filename;
            }
