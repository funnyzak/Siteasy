    ...
    public class Event
    {
        ...
        private void LoadIEvent()
        {
            if (_ievent == null)
            {
                ...
                {
                    EventLogs.WriteFailedLog("定时任务没有定义其 type 属性");
                    ConUtils.InsertLog(3, 0, "", 0, "", "", "定时任务", "执行失败, 定时任务没有定义其 type 属性");
                }

                Type type = Type.GetType(this.ScheduleType);
                if (type == null)
                {
                    EventLogs.WriteFailedLog(string.Format("定时任务 {0} 无法被正确识别", this.ScheduleType));
                    ConUtils.InsertLog(3, 0, "", 0, "", "", "定时任务", string.Format("执行失败, 定时任务 {0} 无法被正确识别", this.ScheduleType));
                }
                else
                {
                    ...
                    if (_ievent == null)
                    {
                        EventLogs.WriteFailedLog(string.Format("定时任务 {0} 未能正确加载", this.ScheduleType));
                        ConUtils.InsertLog(3, 0, "", 0, "", "", "定时任务", string.Format("执行失败, 定时任务 {0} 未能正确加载", this.ScheduleType));
                    }
                }
            }
        }
        ...
        {
            Events.SetLastExecuteScheduledEventDateTime(key, servername, lastexecuted);
        }

        public static DateTime GetLastExecuteScheduledEventDateTime(string key, string servername)
        {
            return Events.GetLastExecuteScheduledEventDateTime(key, servername);
        }
    }
}
