    ...
    public class STACache
    {
        ...
        private STACache()
        {
            ...
            if (applyMemCached || applyRedis)
            {
                try
                {
                    cs = cachedStrategy = (ICacheStrategy)Activator.CreateInstance(Type.GetType("STA.CacLib." + (applyMemCached ? "MemCachedStrategy" : "RedisStrategy") + ", STA.CacLib", false, true));
                }
                catch
                {
                    throw new Exception("请检查STA.CacLib.dll文件是否被放置在bin目录下并配置正确");
                }
            }
            else
            {
                cs = new DefaultCacheStrategy();
                cs.TimeOut = GeneralConfigs.GetConfig().Cacheinterval * 60;
        ...
        /// </summary>
        /// <returns></returns>
        public static STACache GetCacheService()
        {
            if (instance == null)
            {
                lock (lockHelper)
