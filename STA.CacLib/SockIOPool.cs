namespace STA.CacLib
{
    ...
    public class SockIOPool
    {
        ...
        private STA.CacLib.HashingAlgorithm _hashingAlgorithm = STA.CacLib.HashingAlgorithm.Native;
        ...
        private static ResourceManager _resourceManager = new ResourceManager("STA.Cache.MemCached.StringMessages", typeof(SockIOPool).Assembly);
        ...
        public SockIO GetSock(string key, object hashCode)
        {
            ...
            else
            {
                ...
                {
                    case STA.CacLib.HashingAlgorithm.Native:
                        num2 = key.GetHashCode();
                        goto Label_01F0;

                    case STA.CacLib.HashingAlgorithm.OldCompatibleHash:
                        num2 = OriginalHashingAlgorithm(key);
                        goto Label_01F0;

                    case STA.CacLib.HashingAlgorithm.NewCompatibleHash:
                        num2 = NewHashingAlgorithm(key);
                        goto Label_01F0;
                }
                num2 = key.GetHashCode();
                this._hashingAlgorithm = STA.CacLib.HashingAlgorithm.Native;
            }
        Label_01F0:
            while (num++ <= this._buckets.Count)
            {
                ...
                switch (this._hashingAlgorithm)
                {
                    ...
                    case STA.CacLib.HashingAlgorithm.OldCompatibleHash:
                        {
                            num2 += OriginalHashingAlgorithm(num + key);
                            continue;
                        }
                    case STA.CacLib.HashingAlgorithm.NewCompatibleHash:
                        {
                            num2 += NewHashingAlgorithm(num + key);
                            continue;
                        }
                }
                num2 += (num + key).GetHashCode();
        ...
        public STA.CacLib.HashingAlgorithm HashingAlgorithm
        {
            get
            {
