...
namespace STA.Web.Admin
{
    ...
    {
        private string action = STARequest.GetString("hidAction");
        protected void Page_Load(object sender, EventArgs e)
        {
            if (IsPostBack && action != string.Empty)
            {
                ...
                {
                    case "deltag":
                        Message(Contents.DelTag(STARequest.GetFormInt("hidValue", 0)));
                        break;
                }
                hidAction.Value = "";
        ...
        private void LoadData(int pageIndex)
        {
            ...
            DataTable dt = Contents.GetTagDataPage(pageIndex, managelistcount, out pageCount, out recordCount);
