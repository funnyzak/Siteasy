<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="global_tplfiles.aspx.cs" Inherits="STA.Web.Admin.tplfiles" %>
...
<head>
    ...
    <style type="text/css">
        ...
        ul.ftools{padding: 0 0 0 7px;}
        ul.ftools li{float: left;margin: 0 10px 0 0; display: inline;cursor: pointer;}
    </style>
</head>
<body>
    <div id="wrapper">
                                ...
                                <td>
                                    <ul class="ftools">
                                        ...
                                        <li title="压缩当前目录" onclick="SAlert('压缩可能需要一点时间，请稍等...',100);SubmitForm('zipfolder');">
                                            [压缩]</li>
                                        <li title="新建文件" onclick="PopWindow('新建文件 ', '../tools/createnewfile.aspx?path=../../templates/<%=tplpathname%><%=currentvirtualpath%>', '', 605, 300);">
                                            [新建文件]</li>
                                        <li title="在当前目录上传文件" onclick="PopWindow('上传文件', '../tools/uploadfiles.aspx?path=../../templates/<%=tplpathname%><%=currentvirtualpath%>', '', 320, 200);">
                                            [上传文件]</li>
                                        <li title="在当前路径新建文件夹" onclick="PopWindow('创建文件夹', '../tools/createnewfolder.aspx?path=../../templates/<%=tplpathname%><%=currentvirtualpath%>', '', 420, 70);">
                                            [新建文件夹]</li>
                                        <li title="检查当前目录占用空间大小" onclick="SubmitForm('spacesize');">[空间检查]</li>
                                        <li onclick="location.href='global_templatelist.aspx'">[模版管理]</li>
                                    </ul>
                                </td>
                            </tr>
                        </table>
    ...
    <script type="text/javascript">
        ...
        function EditFileName(folder, path) {
            if (folder == "True") {
                PopWindow('修改文件夹 ' + path, '../tools/createnewfolder.aspx?path=../../templates/<%=tplpathname%><%=currentvirtualpath%>' + path, '', 420, 70);
            } else {
                if (IsFileReadType(path)) {
                    PopWindow('修改文件 ' + path, '../tools/createnewfile.aspx?path=../../templates/<%=tplpathname%><%=currentvirtualpath%>' + path, '', 605, 300);
                } else {
