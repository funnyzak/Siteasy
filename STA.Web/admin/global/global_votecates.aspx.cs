...
{
    public partial class votecates : AdminPage
    {
        ...
        private void Del(int id)
        {
            ...
            if (Votes.DelVotecate(id))
                InsertLog(2, "删除投票分类", string.Format("ID:{0},名称:{1}", id, STARequest.GetFormString("name" + id.ToString())));
        }

        private void LoadData(int pageIndex)
        {
            int pageCount, recordCount;
            DataTable dt = Votes.GetVotecateDataPage(pageIndex, managelistcount, out pageCount, out recordCount);
