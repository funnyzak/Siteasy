...
{
    public partial class webcollect : AdminPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ddlConType.BuildTree(Contents.GetChannelDataTable("id,parentid,name"), "name", "id");
                RedirectMessage();
                LoadData(1);
            }
        ...
        private void Del(int id)
        {
            ...
            if (Collects.DelWebCollect(id))
                InsertLog(2, "删除站点信息采集规则", string.Format("ID:{0},名称:{1}", id, STARequest.GetFormString("name" + id.ToString())));
        }

        private void LoadData(int pageIndex)
        {
            int pageCount, recordCount;
            DataTable dt = Collects.GetWebCollectDataTable(pageIndex, TypeParse.StrToInt(ViewState["pagesize"], managelistcount), TypeParse.ObjToString(ViewState["condition"]), out pageCount, out recordCount);
        ...
        {
            ViewState["condition"] = Collects.GetWebCollectSearchCondition(txtKeywords.Text, TypeParse.StrToInt(ddlConType.SelectedValue), "", "");
