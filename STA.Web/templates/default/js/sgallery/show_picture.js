...
{
	var oTxtBtn=document.getElementById('disp_text');
	var oTxtCon=document.getElementById('text_warp');
	oTxtBtn.onclick=function()
	{
		
		if(oTxtCon.className=='active')
		{
			oTxtCon.className='normal';
			oTxtBtn.style.backgroundPosition='0 -50px';
		}
		else
		{
			oTxtCon.className='active';
			oTxtBtn.style.backgroundPosition='0 0';
		}
	}
}
$(document).ready(function(){
	$("#nhp_poparea a.nhp_pclose").click(function(){
		$("#nhp_poparea").addClass("normal");
		})
	//获取锚点即当前图片id
	...
	//$('#big-pic img').LoadImage(true, 1000,$("#load_pic").attr('rel'));
	//$("#picnum").html("("+picid+"/"+totalnum+")");
	$("#picinfo").html($("#pictureurls li:nth-child("+picid+") img").attr("alt"));
	
	var curnum=0;
	for(curnum=0;curnum<totalnum+1;curnum++)
	{
		$("#pictureurls li:nth-child("+curnum+") .img-wrap span").html(curnum+"/"+totalnum);
	}
