<%@ Page language="c#" AutoEventWireup="false" EnableViewState="false" Inherits="STA.Page.User.ContentList" %>
...
override protected void OnInit(EventArgs e)
{
	...
	*/
	...
	DataTable clist = Contents.GetContentDataPage("id,savepath,filename,title,status,addtime,updatetime,channelid,channelname","",page,9, Contents.GetContentSearchCondition(typeid,username,0,0, -1, "", "", "",""),"updatetime desc",out pagecount, out recordcount);
	
	templateBuilder.Append("\r\n                <form name=\"zform\" id=\"zform\" action=\"\" method=\"post\">\r\n                    ");
	if (clist.Rows.Count<=0)
	{

	templateBuilder.Append("\r\n                    <div class=\"zerotable\">您还没有投过稿,现在就去<a href=\"");
	templateBuilder.Append(siteurl.ToString());
	templateBuilder.Append("/user/con_add.aspx\" class=\"cblue s13\">投稿</a></div>\r\n                    ");
	}
	else
	{
	...
	templateBuilder.Append("\r\n                            <tr>\r\n                                <td width=\"5%\" align=\"center\"><input type=\"checkbox\" name=\"cbid\" value=\"" + item["id"].ToString().Trim() + "\" onclick=\"checkedEnabledButton(this.form, 'cbid', 'btndelete' , 'btnedit', 'btnviewcomment')\"/></td>\r\n                                <td width=\"3%\" align=\"center\"><img src=\"");
	templateBuilder.Append(tempurl.ToString());
	...
	templateBuilder.Append("\" title=\"" + item["title"].ToString().Trim() + "\" target=\"_blank\" class=\"cblue\">");	templateBuilder.Append(Utils.GetUnicodeSubString(item["title"].ToString().Trim(),56,".."));
	templateBuilder.Append("</a></td>\r\n                                <td width=\"15%\" align=\"center\"><a href=\"");
	templateBuilder.Append(Urls.Channel(chid).ToString().Trim());
	templateBuilder.Append("\" title=\"" + item["channelname"].ToString().Trim() + "\" class=\"cgrey\" target=\"_blank\">" + item["channelname"].ToString().Trim() + "</a></td>\r\n                                <td width=\"15%\" align=\"center\"><span class=\"posttime cgrey\" title=\"");	templateBuilder.Append(Convert.ToDateTime(Convert.ToDateTime(item["updatetime"].ToString().Trim())).ToString(" yyyy-MM-dd HH:mm:ss"));
	templateBuilder.Append("\"></span></td>\r\n                                ");	string status = ((ConStatus)TypeParse.StrToInt(item["status"])).ToString();
	
	templateBuilder.Append("\r\n                                <td width=\"10%\" align=\"center\"");
	if (status!="通过")
	{

	templateBuilder.Append(" class=\"cred\"");
	}	//end if

	templateBuilder.Append(">");
	templateBuilder.Append(status.ToString());
	templateBuilder.Append("</td>\r\n                            </tr>\r\n                            ");
	}	//end loop

	templateBuilder.Append("\r\n                        </tbody>\r\n                    </table>\r\n                	<div class=\"btn-manage\">\r\n                    	<input name=\"btndelete\" id=\"btndelete\" type=\"button\" class=\"button\" value=\"删除选中\" onclick=\"delCons(getCbCheckedVals(),'");
	templateBuilder.Append(siteurl.ToString());
	templateBuilder.Append("/user/con_list.aspx')\" disabled=\"disabled\"/>&nbsp;\r\n                        <input name=\"btnedit\" id=\"btnedit\" type=\"button\" class=\"button\" value=\"编辑\" disabled=\"disabled\"/>&nbsp;\r\n                        <input name=\"btnviewcomment\" id=\"btnviewcomment\" type=\"button\" class=\"button\" value=\"查看评论\" disabled=\"disabled\"/>&nbsp;\r\n                        <input name=\"btn3\" onclick=\"location.href='");
	templateBuilder.Append(siteurl.ToString());
	templateBuilder.Append("/");
	templateBuilder.Append(ctinfo.addmod.ToString().Trim());
	templateBuilder.Append("?type=");
	templateBuilder.Append(typeid.ToString());
	templateBuilder.Append("'\" type=\"button\" class=\"button\" value=\"投稿\"/>&nbsp;\r\n                    </div> \r\n                        ");
	if (pagecount>1)
	{

	string pagenumbers = PageNumber(siteurl+"/user/con_list.aspx",siteurl+"/user/con_list.aspx?typeid="+typeid+"&page=@page",page,pagecount,recordcount,7, false).ToString();
