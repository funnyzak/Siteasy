<%@ Page language="c#" AutoEventWireup="false" EnableViewState="false" Inherits="STA.Page.User.Inboxpm" %>
...
override protected void OnInit(EventArgs e)
{
	...
	*/
	...
	DataTable msglist = STA.Core.PrivateMessages.GetPrivateMessageList(userid,0,9,page,2,out pagecount,out recordcount);
	
	templateBuilder.Append("\r\n                <form name=\"zform\" id=\"zform\" action=\"\" method=\"post\">\r\n                    ");
	if (msglist.Rows.Count<=0)
	{

	templateBuilder.Append("\r\n                    <div class=\"zerotable\">收件箱暂无短信息</div>\r\n                    ");
	}
	else
	{
	...
	templateBuilder.Append(siteurl.ToString());
	templateBuilder.Append("/user/readpm.aspx?pmid=" + item["id"].ToString().Trim() + "&from=inbox\" class=\"cblue");
	if (item["new"].ToString().Trim()=="1")
	{
	...
	templateBuilder.Append(siteurl.ToString());
	templateBuilder.Append("/user/inboxpm.aspx')\" disabled=\"disabled\"/>&nbsp;\r\n                        <input name=\"btnread\" id=\"btnread\" type=\"button\" class=\"button\" value=\"标记为已读\" disabled=\"disabled\" onclick=\"setPmsState(getCbCheckedVals(),0,'");
	templateBuilder.Append(siteurl.ToString());
	templateBuilder.Append("/user/inboxpm.aspx')\"/>&nbsp;&nbsp;\r\n                    </div> \r\n                        ");
	if (pagecount>1)
	{

	string pagenumbers = PageNumber(siteurl+"/user/inboxpm.aspx",siteurl+"/user/inboxpm.aspx?page=@page",page,pagecount,recordcount,7, false).ToString();
