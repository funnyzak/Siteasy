...
override protected void OnInit(EventArgs e)
{
	...
	*/
	...
	templateBuilder.Append("</h1>\r\n                </div>\r\n                <div class=\"softinfo clearfix\">\r\n                    <div class=\"softsummary\">\r\n                        ");	SelectInfo nativeplace = SelectByVal("nativeplace",info.Ext["ext_nativeplace"].ToString());
	
	SelectInfo infotype = SelectByVal("infotype",info.Ext["ext_infotype"].ToString());
	
	templateBuilder.Append("\r\n                        <label>所属地区：</label><span class=\"native\">");
	templateBuilder.Append(nativeplace.Name.ToString().Trim());
	templateBuilder.Append("</span>\r\n                        <label>信息类型：</label><span class=\"infotype\">");
	templateBuilder.Append(infotype.Name.ToString().Trim());
	templateBuilder.Append("</span>\r\n                        <label>截止日期：</label><span>");	templateBuilder.Append(Convert.ToDateTime(Convert.ToDateTime(info.Ext["ext_endtime"].ToString().Trim())).ToString(" yyyy-MM-dd"));
	templateBuilder.Append("</span>\r\n                        <label>更新时间：</label><span>");	templateBuilder.Append(Convert.ToDateTime(Convert.ToDateTime(info.Updatetime)).ToString(" yyyy-MM-dd"));
	templateBuilder.Append("</span>\r\n                        <label>联系人：</label><span>" + info.Ext["ext_linkman"].ToString().Trim() + "</span>\r\n                        <label>联系电话：</label><span>" + info.Ext["ext_tel"].ToString().Trim() + "</span>\r\n                        <label>联系邮件：</label><span>" + info.Ext["ext_email"].ToString().Trim() + "</span>\r\n                        <label>联系地址：</label><span>" + info.Ext["ext_address"].ToString().Trim() + "</span>\r\n                        <label>浏览次数：</label><span class=\"conclick\">");
	templateBuilder.Append(info.Click.ToString().Trim());
	templateBuilder.Append("</span>\r\n                    </div>\r\n                    <script type=\"text/javascript\">\r\n                        $(\"span.native\").html(getSelectStrFromData(\"nativeplace\", \"" + info.Ext["ext_nativeplace"].ToString().Trim() + "\"));\r\n                        $(\"span.infotype\").html(getSelectStrFromData(\"infotype\", \"" + info.Ext["ext_infotype"].ToString().Trim() + "\"));\r\n                    </");
	templateBuilder.Append("script>\r\n                    <div class=\"softsummarypic\">\r\n                         ");	string img = info.Img==""?(tempurl+ "/images/pub/nopic.png"):info.Img;
	
	templateBuilder.Append("\r\n                        <img src='");
	templateBuilder.Append(img.ToString());
	templateBuilder.Append("' onerror=\"this.src='");
	templateBuilder.Append(tempurl.ToString());
	templateBuilder.Append("/images/pub/nopic.png'\"/>\r\n                    </div>\r\n				</div>\r\n				<div class=\"info-intro fyahei s16 bold\">详细信息</div>\r\n                <div class=\"pcontent\" style=\"width:670px;padding-left:10px;\">\r\n                     ");
