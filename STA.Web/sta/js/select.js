...
function ChangeSon() {
    ...
    document.getElementById('ext_'+emname).value = topSelObj.options[topSelObj.selectedIndex].value;
    }
    else {
    document.getElementById('ext_'+emname).value = this.options[this.selectedIndex].value;
    }
    */
    ...
    document.getElementById('ext_' + emname).value = v;
    var newobj = oj.options;
    var selarr = eval('em_' + emname + 's');

    var selv = parseInt(v);
    var maxv = parseInt(v) + 0.999;
...
function MakeTopSelect(emname, selvalue) {
    ...
    document.getElementById("span_" + emname).innerHTML = document.getElementById("span_" + emname + "_son").innerHTML = document.getElementById("span_" + emname + "_sec").innerHTML = "";
    document.getElementById('ext_' + emname).value = selvalue;
    ...
    if (secvalue > 0) {
        ...
        var selv = sonvalue;
        var maxv = parseInt(sonvalue) + 0.999;
        //alert(selv + "__" + maxv);
        i = 0;
        for (i = selv + 0.001; i < maxv; i = FloatAdd(i, 0.001)) {
            if (!selarr[i]) continue;
...
//返回select的串联对象 
function getSelectFromData(name, val) {
    var arr = eval('em_' + name + 's'), ret = { top: "", son: "", sec: "" };
    if (val % 500 == 0) {
        ret.top = arr[val];
    } else {
        ret.top = arr[val - (val % 500)]
        ret.son = arr[Math.floor(val)]
        ret.sec = (val % 1 == 0) ? "" : arr[val];
    }
    return ret;
};

function getSelectStrFromData(name, val) {
    var o = getSelectFromData(name, val);
    return (o.top + " " + o.son + " " + o.sec).trim();
};

function getSelectTextFromData(name, val) {
    return eval("em_" + name + "s[" + val + "]");
};