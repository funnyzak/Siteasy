//======================================================
//==     (c)2009 STASystem inc by fSTA v1.0           ==
//==          mail:f@funnySTA.com                     ==
//==         Website:www.funnySTA.com                 ==
//======================================================
using System;
using System.Web;
...
namespace STA.Control
{
    ...
    public class UpFile : System.Web.UI.WebControls.WebControl,IWebControl
    {
        /// <summary>
        /// TextBox 控件变量
        /// </summary>
        ...
        public UpFile()
        {
            ...
            tb.Style.Add("cursor", "pointer");
            tb.Attributes.Add("ondblclick", "if(this.value==''){return false;}window.open(this.value);");
        ...
        /// 设置限制上传文件的大小
        /// </summary>
        [Bindable(true), Category("Appearance"), DefaultValue(5120)]
        public int FileMaxLength
        {
            get
            {
                object o = ViewState["FileMaxLength"];
                return (o == null) ? 5120 : Convert.ToInt32(o);
            }
            set
            {
                ViewState["FileMaxLength"] = value;
            }
        }
        /// <summary>
        /// 返回上传后文件的大小
        ...
        /// <summary>
        /// 此为文件所在服务器的虚拟地址,不用设置 如 /upload/1.jpg
        /// </summary>
        public string FileVirtualPath
        {
            get
            {
                object o = ViewState["FileVirtualPath"];
                return (o == null) ? "" : o.ToString();
            }
            set
            {
                ViewState["FileVirtualPath"] = value;
            }
        }
        ...
        /// <summary>
        /// js函数回调返回 Web地址和所在服务器的虚拟路径
        /// </summary>
        [Bindable(true), Category("Appearance"), DefaultValue("")]
        public string JsUploadCallBack
        {
            get
            {
                object o = ViewState["JsUploadCallBack"];
                return (o == null) ? "" : o.ToString()+"(\"{s}\");";
            }
            set
            {
                ViewState["JsUploadCallBack"] = value;
            }
        }
        /// <summary>
        /// js函数回调返回上传后的信息  
        /// </summary>
        [Bindable(true), Category("Appearance"), DefaultValue("")]
        public string JsInfoCallBack
        {
            get
            {
                object o = ViewState["JsInfoCallBack"];
                return (o == null) ? "" : o.ToString() + "(\"{s}\");";
            }
            set
            {
                ViewState["JsInfoCallBack"] = value;
            }
        }
        /// <summary>
        /// 上传后所记入数据库时所引用的路径 如 http://www.funnySTA.com/upload/
        /// </summary>
        [Bindable(true), Category("Appearance"), DefaultValue("")]
        ...
        protected override void Render(HtmlTextWriter output)
        {
            ...
            if (this.Tips != "null")
            {
                tb.Attributes.Add("onmouseover", "ztip.s('" + Tips + "');this.focus();this.select();");
                tb.Attributes.Add("onmouseout", "ztip.h()");
            }
            CreateChildControls();
        ...
        public string UpdateFile()
        {
            ...
            if (!string.IsNullOrEmpty(JsInfoCallBack)) { this.Msglabel.Visible = false; }
            if (FileUpload.PostedFile != null)
            {
                HttpPostedFile myFile = FileUpload.PostedFile;
                int nFileLen = myFile.ContentLength;
                string _fileext = System.IO.Path.GetExtension(myFile.FileName).ToLower();
                if (nFileLen == 0)
                {
                    Msglabel.Text = "&nbsp;&nbsp;<font color=red>没有选定被上传的文件</font></b>";
                    JsInfoCall(Msglabel.Text);
                    return "";
                }
                
                if (FileType.IndexOf(_fileext) < 0)
                {
                    Msglabel.Text = "&nbsp;&nbsp;<font color=red>文件必须是以" + FileType.Replace("|", " , ") + "为扩展名的文件</font></b>";
                    JsInfoCall(Msglabel.Text);
                    return "";
                }
                if (nFileLen > FileMaxLength*1024)
                {
                    Msglabel.Text = "&nbsp;&nbsp;<font color=red>文件必须控制在" + FileMaxLength.ToString() + "KB以内</font></b>";
                    JsInfoCall(Msglabel.Text);
                    return "";
                }
                ...
                FileVirtualPath = UpFilePath + sFilename;
                FileSize = (myFile.ContentLength / 1024).ToString();
                //上传图片文件jpg,gif
        ...
        {
            this.Page.ClientScript.RegisterStartupScript(this.Page.GetType(), "clientjs", js);
        }
        private void JsOkCallBack()
        {
            if (JsUploadCallBack != "") { ExecuteJs("<script>" + JsUploadCallBack.Replace("{s}", Text+","+FileVirtualPath) + "</script>"); }
        }
        private void JsInfoCall(string Info)
        {
            if (JsInfoCallBack != "") { ExecuteJs("<script>" + JsInfoCallBack.Replace("{s}", Info) + "</script>"); }
        }
        /// <summary>
        /// 上传单击事件
        /// </summary>
