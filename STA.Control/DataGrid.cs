...
namespace STA.Control
{
    ...
    public class DataGrid : System.Web.UI.WebControls.DataGrid, INamingContainer
    {
        ...
        /// 跳转按钮
        /// </summary>
        public STA.Control.Button GoToPagerButton = new STA.Control.Button();
        ...
        public void BindData(string sqlstring)
        {
            ...
            DataTable dt = DbHelper.ExecuteDataset(sqlstring).Tables[0];
            BindData(dt);
        }
        ...
        public void SortTable(string SortExpression, string sqlstring)
        {
            ...
            {
                dv = new DataView(DbHelper.ExecuteDataset(sqlstring).Tables[0]);
            }
            else
            {
                return;
                ...
                if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    for (int i = 0; i < e.Item.Cells.Count; i++)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                //加载STA下拉控件
                                if (c is STA.Control.DropDownList)
                                {
                                    STA.Control.DropDownList dropDownList = (STA.Control.DropDownList)c;
        ...
        public System.Collections.ArrayList GetKeyIDArray()
        {
            ...
            if (STARequest.GetString("keyid") != "")
            {
                foreach (string id in STARequest.GetString("keyid").Split(','))
                {
                    al.Add(id);
                }
            }
        ...
        {
            return STARequest.GetFormString(this.ClientID.Replace("_", ":") + ":_ctl" + (controlnumber + 3) + ":" + fieldname);
        }
