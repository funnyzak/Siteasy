    ...
    public partial class DataProvider : IDataProvider
    {
        ...
        private DbParameter[] EntityContent(ContentInfo info)
        {
            return new DbParameter[] 
            {
                ...
                DbHelper.MakeInParam("@title",(DbType)SqlDbType.NVarChar,255,info.Title),
                DbHelper.MakeInParam("@subtitle",(DbType)SqlDbType.NVarChar,255,info.Subtitle),
        ...
        public IDataReader GetShortContent(int id)
        {
        ...
        public IDataReader GetContent(int id, int typeid, ref string extfields)
        {
            DataRow[] tdrs = STA.Core.Caches.GetContypeTable(GeneralConfigs.GetConfig().Cacheinterval * 60).Select("id=" + typeid.ToString());
            if (tdrs.Length <= 0)
            {
                return DbHelper.ExecuteReader(CommandType.Text, string.Format("SELECT * FROM {0}contents WHERE id = {1}", BaseConfigs.GetTablePrefix, id.ToString()));
            }
            ...
            {
                extfields = tdrs[0]["fields"].ToString().Trim();
                string fields = ("con." + DbFields.Content.Replace(", ", ", con.") + "," + extfields).TrimEnd(',');
                return DbHelper.ExecuteReader(CommandType.Text, string.Format("SELECT {0} FROM [{1}contents] con INNER JOIN {2} ext ON con.id = ext.cid WHERE con.id = {3}",
                                             fields,
                                             BaseConfigs.GetTablePrefix,
                                             tdrs[0]["extable"].ToString().Trim(),
                                             id.ToString()));
            }
        }
        ...
        private DbParameter[] EntityChannel(ChannelInfo info)
        {
            return new DbParameter[] 
            {
                ...
                DbHelper.MakeInParam("@name",(DbType)SqlDbType.NVarChar,255,info.Name),
        ...
        public DataTable GetAttachmentDataPage(string fields, int pagecurrent, int pagesize, string condtion, out int pagecount, out int recordcount)
        {
            return GetDataPage(string.Format("[{0}attachments]", BaseConfigs.GetTablePrefix), "id", pagecurrent, pagesize, fields == "" ? "*" : fields, "id desc", condtion, out pagecount, out recordcount);
        }
        ...
        public int DelComment(int id, int ctype)
        {
            return DbHelper.ExecuteNonQuery(CommandType.StoredProcedure, string.Format("{0}deletecomment", BaseConfigs.GetTablePrefix), DbHelper.MakeInParam("@id", (DbType)SqlDbType.Int, 4, id), DbHelper.MakeInParam("@ctype", (DbType)SqlDbType.TinyInt, 1, (int)ctype));
        }
        ...
        public int DelCommentByCid(int cid, int ctype)
        {
            return DbHelper.ExecuteNonQuery(string.Format("DELETE FROM [{0}comments] WHERE [{0}comments].[cid] = {1} AND [{0}comments].[ctype] = {2}", BaseConfigs.GetTablePrefix, cid, ctype));
        }
        ...
        public string GetCommentSearchCondition(int status, int ctype, int cid, string users, string ip, string startdate, string enddate, string contitle, string keywords)
        {
            StringBuilder condition = new StringBuilder("id>0");
            if (ctype > 0)
                condition.AppendFormat(" AND ctype = {0}", ctype);
            if (status > -1)
        ...
        private DbParameter[] EntityComment(CommentInfo info)
        {
            return new DbParameter[] { 
                DbHelper.MakeInParam("@id",(DbType)SqlDbType.Int,4,info.Id),
                DbHelper.MakeInParam("@ctype",(DbType)SqlDbType.TinyInt,1,(int)info.Ctype),
                DbHelper.MakeInParam("@cid",(DbType)SqlDbType.Int,4,info.Cid),
                ...
                DbHelper.MakeInParam("@useragent",(DbType)SqlDbType.NVarChar,200,info.Useragent)
            };
        }
        ...
        public string CommentSummary(int ctype, int cid)
        {
            DbParameter summary = DbHelper.MakeOutParam("@summary", (DbType)SqlDbType.NVarChar, 50);
            DbParameter[] parms = {
                DbHelper.MakeInParam("@ctype",(DbType)SqlDbType.TinyInt,1,ctype),
                DbHelper.MakeInParam("@cid",(DbType)SqlDbType.Int,4,cid),summary
			};
            DbHelper.ExecuteNonQuery(CommandType.StoredProcedure, string.Format("{0}commentsummary", BaseConfigs.GetTablePrefix), parms);
            return TypeParse.ObjToString(summary.Value, "");
        }
        ...
        {
            StringBuilder condition = new StringBuilder("con.id>" + minid.ToString());
            if (maxid > 0)
                condition.AppendFormat(" AND con.id < {0}", maxid.ToString());
            if (startdate != string.Empty)
                condition.AppendFormat(" AND con.addtime >= '{0}'", TypeParse.StrToDateTime(startdate));
            if (enddate != string.Empty)
                condition.AppendFormat(" AND con.addtime <= '{0}'", TypeParse.StrToDateTime(enddate).AddDays(1));
            if (channelid > 0)
                condition.AppendFormat(" AND (con.channelid = {0} or con.channelfamily like '%,{0},%' or con.extchannels like '%,{0},%')", channelid.ToString());
            return condition.ToString();
        }

        public DataTable GetPublishChannelTable(string ids)
        {
            string commandText = string.Format("SELECT [id],[ctype],[savepath],[filename],[listrule],[content],[listcount] FROM [{0}channels] WHERE ctype != 3 AND viewgroup = '' AND ipaccess = '' AND ipdenyaccess = '' {1}",
                                 BaseConfigs.GetTablePrefix,
        ...
        {
            return DbHelper.ExecuteDataset(string.Format("SELECT con.[id],con.[savepath],con.[filename],con.[content] FROM [{0}contents] con LEFT JOIN [{0}channels] chl ON con.channelid = chl.id WHERE con.typeid > 0 AND con.orderid>=-1000 AND con.status=2 AND ISNULL(chl.viewcongroup,'') = '' AND con.credits = 0 AND ISNULL(chl.ipaccess,'') = '' AND ISNULL(chl.ipdenyaccess,'') = '' {1}",
                                   BaseConfigs.GetTablePrefix,
        ...
        public DataTable GetDbTable(string table, int num, string where, string fields, string order)
        {
            if (table.Trim() == "") return null;
            string commandText = string.Format("SELECT TOP {0} {1} FROM {2} {3} {4}", num <= 0 ? 100 : num, fields == "" ? "*" : fields, table, where.Trim() == "" ? "" : ("WHERE " + where), order == "" ? "" : ("ORDER BY " + order));
            return DbHelper.ExecuteDataset(commandText).Tables[0];
        }
        ...
        public DataTable GetUIVoteData(DataParmInfo info)
        {
            try
            {
                ...
                if (info.Durdate > 0)
                    where += string.Format(" AND [{0}votetopics].[addtime] >= '{1}'", tableprefix, DateTime.Now.AddDays(-info.Durdate).ToString("yyyy-MM-dd HH:mm:ss"));
        ...
        public DataTable GetUICommentData(DataParmInfo info)
        {
            try
            {
                ...
                if (info.Uid > 0)
                    where += string.Format(" AND [{0}comments].[uid] = {1}", tableprefix, info.Uid);

                where += string.Format(" AND [{0}comments].[ctype] = {1}", tableprefix, info.Ctype <= 0 ? 1 : info.Ctype);

                if (info.Durdate > 0)
                    where += string.Format(" AND [{0}comments].[addtime] >= '{1}'", tableprefix, DateTime.Now.AddDays(-info.Durdate).ToString("yyyy-MM-dd HH:mm:ss"));
        ...
        #region Word
        private DbParameter[] EntityWord(WordInfo info)
        {
            return new DbParameter[] { 
				DbHelper.MakeInParam("@id",(DbType)SqlDbType.Int,4,info.Id),
				DbHelper.MakeInParam("@uid",(DbType)SqlDbType.Int,4,info.Uid),
				DbHelper.MakeInParam("@username",(DbType)SqlDbType.NChar,20,info.Username),
				DbHelper.MakeInParam("@find",(DbType)SqlDbType.NVarChar,255,info.Find),
				DbHelper.MakeInParam("@replacement",(DbType)SqlDbType.NVarChar,255,info.Replacement)
            };
        }

        public int AddWord(WordInfo info)
        {
            string commandText = string.Format("INSERT INTO [{0}words] ([uid], [username], [find], [replacement]) VALUES (@uid, @username, @find, @replacement);SELECT SCOPE_IDENTITY()", BaseConfigs.GetTablePrefix);
            return TypeParse.StrToInt(DbHelper.ExecuteScalar(CommandType.Text, commandText, EntityWord(info)));
        }

        public int EditWord(WordInfo info)
        {
            string commandText = string.Format("UPDATE [{0}words] SET [uid] = @uid, [username] = @username, [find] = @find, [replacement] = @replacement WHERE [{0}words].[id] = @id", BaseConfigs.GetTablePrefix);
            return DbHelper.ExecuteNonQuery(CommandType.Text, commandText, EntityWord(info));
        }

        public int DelWord(int id)
        {
            return DbHelper.ExecuteNonQuery(string.Format("DELETE FROM [{0}words] WHERE [{0}words].[id] = {1}", BaseConfigs.GetTablePrefix, id));
        }
        ...
        public IDataReader GetWord(int id)
        {
            return DbHelper.ExecuteReader(CommandType.Text, string.Format("SELECT TOP 1 * FROM [{0}words] WHERE [{0}words].[id] = {1}", BaseConfigs.GetTablePrefix, id));
        }

        public DataTable GetWordDataPage(string fields, int pagecurrent, int pagesize, string condition, out int pagecount, out int recordcount)
        {
            return GetDataPage(string.Format("{0}words", BaseConfigs.GetTablePrefix), "id", pagecurrent, pagesize, fields == "" ? "*" : fields, "id desc", condition, out pagecount, out recordcount);
        }

        public int DelWords(string idList)
        {
