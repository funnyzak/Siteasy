...
namespace STA.Data.SqlServer
{
    public partial class DataProvider : IDataProvider
    {
        ...
        public string BackUpDatabase(string backUpPath, string serverName, string userName, string password, string strDbName, string strFileName)
        {
            ...
            try
            {
                if (FileUtil.DirExists(backUpPath))
                    FileUtil.CreateFolder(backUpPath);

                string backsql = string.Format("BACKUP DATABASE [{0}] to DISK ='{1}'", strDbName, backUpPath + strFileName + ".config");
                DbHelper.ExecuteNonQuery(backsql);
                return string.Empty;
            ...
            {
                STAException.WriteError(Utils.GetMapPath(BaseConfigs.GetSitePath + "/sta/logs/error"), "数据库备份失败！", ex);
        ...
        #region Appconnect
        private DbParameter[] EntityAppconnect(AppconnectInfo info)
        {
            return new DbParameter[] { 
				DbHelper.MakeInParam("@id",(DbType)SqlDbType.Int,4,info.Id),
				DbHelper.MakeInParam("@name",(DbType)SqlDbType.NVarChar,50,info.Name),
				DbHelper.MakeInParam("@description",(DbType)SqlDbType.NVarChar,1000,info.Description),
				DbHelper.MakeInParam("@identify",(DbType)SqlDbType.NVarChar,20,info.Identify),
				...
				DbHelper.MakeInParam("@appid",(DbType)SqlDbType.NVarChar,50,info.Appid),
				DbHelper.MakeInParam("@appkey",(DbType)SqlDbType.NVarChar,50,info.Appkey),
				DbHelper.MakeInParam("@orderid",(DbType)SqlDbType.Int,4,info.Orderid),
				DbHelper.MakeInParam("@ext",(DbType)SqlDbType.NVarChar,500,info.Ext)
            };
        }

        public int AddAppconnect(AppconnectInfo info)
        {
            string commandText = string.Format("INSERT INTO [{0}appconnects] ([name], [description], [identify], [isvalid], [img], [appid], [appkey], [orderid], [ext]) VALUES (@name, @description, @identify, @isvalid, @img, @appid, @appkey, @orderid, @ext);SELECT SCOPE_IDENTITY()", BaseConfigs.GetTablePrefix);
            return TypeParse.StrToInt(DbHelper.ExecuteScalar(CommandType.Text, commandText, EntityAppconnect(info)));
        }

        public int EditAppconnect(AppconnectInfo info)
        {
            string commandText = string.Format("UPDATE [{0}appconnects] SET [name] = @name, [description] = @description, [identify] = @identify, [isvalid] = @isvalid, [img] = @img, [appid] = @appid, [appkey] = @appkey, [orderid] = @orderid, [ext] = @ext WHERE [{0}appconnects].[id] = @id", BaseConfigs.GetTablePrefix);
            return DbHelper.ExecuteNonQuery(CommandType.Text, commandText, EntityAppconnect(info));
        }

        public int DelAppconnect(int id)
        {
            return DbHelper.ExecuteNonQuery(string.Format("DELETE FROM [{0}appconnects] WHERE [{0}appconnects].[id] = {1}", BaseConfigs.GetTablePrefix, id));
        }

        public IDataReader GetAppconnect(int id)
        {
            return DbHelper.ExecuteReader(CommandType.Text, string.Format("SELECT TOP 1 * FROM [{0}appconnects] WHERE [{0}appconnects].[id] = {1}", BaseConfigs.GetTablePrefix, id));
        }

        public IDataReader GetAppconnect(string identify)
        {
            return DbHelper.ExecuteReader(CommandType.Text, string.Format("SELECT TOP 1 * FROM [{0}appconnects] WHERE [{0}appconnects].[identify] = @identify", BaseConfigs.GetTablePrefix), DbHelper.MakeInParam("@identify", (DbType)SqlDbType.NVarChar, 20, identify));
        }

        public DataTable GetAppconnectDataPage(string fields, int pagecurrent, int pagesize, string condition, out int pagecount, out int recordcount)
        {
            return GetDataPage(string.Format("{0}appconnects", BaseConfigs.GetTablePrefix), "id", pagecurrent, pagesize, fields == "" ? "*" : fields, "id desc", condition, out pagecount, out recordcount);
        }
        #endregion

        #region Userconnect
        private DbParameter[] EntityUserconnect(UserconnectInfo info)
        {
            return new DbParameter[] { 
				DbHelper.MakeInParam("@id",(DbType)SqlDbType.Int,4,info.Id),
				DbHelper.MakeInParam("@uid",(DbType)SqlDbType.Int,4,info.Uid),
				DbHelper.MakeInParam("@appidentify",(DbType)SqlDbType.NVarChar,20,info.Appidentify),
				DbHelper.MakeInParam("@openid",(DbType)SqlDbType.NVarChar,50,info.Openid),
				DbHelper.MakeInParam("@token",(DbType)SqlDbType.NVarChar,50,info.Token),
				DbHelper.MakeInParam("@secret",(DbType)SqlDbType.NVarChar,50,info.Secret),
				DbHelper.MakeInParam("@ext",(DbType)SqlDbType.NVarChar,500,info.Ext),
				DbHelper.MakeInParam("@addtime",(DbType)SqlDbType.DateTime,8,info.Addtime)
            };
        }

        public int AddUserconnect(UserconnectInfo info)
        {
            string commandText = string.Format("INSERT INTO [{0}userconnects] ([uid], [appidentify], [openid], [token], [secret], [ext], [addtime]) VALUES (@uid, @appidentify, @openid, @token, @secret, @ext, @addtime);SELECT SCOPE_IDENTITY()", BaseConfigs.GetTablePrefix);
            return TypeParse.StrToInt(DbHelper.ExecuteScalar(CommandType.Text, commandText, EntityUserconnect(info)));
        }

        public int EditUserconnect(UserconnectInfo info)
        {
            string commandText = string.Format("UPDATE [{0}userconnects] SET [uid] = @uid, [appidentify] = @appidentify, [openid] = @openid, [token] = @token, [secret] = @secret, [ext] = @ext, [addtime] = @addtime WHERE [{0}userconnects].[id] = @id", BaseConfigs.GetTablePrefix);
            return DbHelper.ExecuteNonQuery(CommandType.Text, commandText, EntityUserconnect(info));
        }

        public int DelUserconnect(int id)
        {
            return DbHelper.ExecuteNonQuery(string.Format("DELETE FROM [{0}userconnects] WHERE [{0}userconnects].[id] = {1}", BaseConfigs.GetTablePrefix, id));
        }

        public IDataReader GetUserconnect(int uid, string identify)
        {
            return DbHelper.ExecuteReader(CommandType.Text, string.Format("SELECT TOP 1 * FROM [{0}userconnects] WHERE [{0}userconnects].[uid] = {1} AND [{0}userconnects].[appidentify] = '{2}'", BaseConfigs.GetTablePrefix, uid, identify));
        }

        public IDataReader GetUserconnect(string openid, string identify)
        {
            return DbHelper.ExecuteReader(CommandType.Text, string.Format("SELECT TOP 1 * FROM [{0}userconnects] WHERE [{0}userconnects].[openid] = '{1}' AND [{0}userconnects].[appidentify] = '{2}'", BaseConfigs.GetTablePrefix, openid, identify));
        }

        public DataTable GetUserconnectDataPage(string fields, int pagecurrent, int pagesize, string condition, out int pagecount, out int recordcount)
        {
            return GetDataPage(string.Format("{0}userconnects", BaseConfigs.GetTablePrefix), "id", pagecurrent, pagesize, fields == "" ? "*" : fields, "id desc", condition, out pagecount, out recordcount);
        }
        #endregion
