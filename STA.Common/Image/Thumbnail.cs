...
namespace STA.Common.ImgHelper
{
    ...
    public class Thumbnail
    {
        ...
        {
            return ResizeImage(width, height, maxWidth, maxHeight, false);
        }
        /// <summary>
        /// 计算新尺寸
        /// </summary>
        ...
        /// <param name="ignoreoldsize">是否忽略原图尺寸</param>
        /// <returns></returns>
        private static Size ResizeImage(int width, int height, int maxWidth, int maxHeight, bool ignoreoldsize)
        {
            ...
            if (originalWidth > MAX_WIDTH || originalHeight > MAX_HEIGHT || ignoreoldsize)
            {
                decimal factor;
        ...
        /// <param name="srcRect">图片对象需要截取的地方</param>
        public static void MakeSquareImage(Image image, string newFileName, int newSize, Rectangle srcRect)
        {
            ...
            try
            {
                ...
                g.DrawImage(image, new Rectangle(0, 0, newSize, newSize), srcRect, GraphicsUnit.Pixel);
        ...
        ///  <param name="ignoresize">是否忽略原尺寸进行比例缩放</param>
        public static void MakeThumbnailImage(Image original, string newFileName, int maxWidth, int maxHeight, bool ignoresize)
        {
            Size _newSize = ResizeImage(original.Width, original.Height, maxWidth, maxHeight, ignoresize);
        ...
        /// <param name="maxHeight">最大高度</param>
        /// <param name="ignoresize">是否忽略原尺寸进行比例缩放</param>
        public static Image GetThumbnailImage(Image original, int maxWidth, int maxHeight, bool ignoresize)
        {
            Size _newSize = ResizeImage(original.Width, original.Height, maxWidth, maxHeight, ignoresize);
            Image displayImage = new Bitmap(original, _newSize);
        ...
        {
            MakeThumbnailImage(Image.FromFile(fileName), newFileName, maxWidth, maxHeight, false);
        }
        ...
        public static void MakeRemoteThumbnailImage(string url, string newFileName, int maxWidth, int maxHeight)
        {
            ...
            Image original = Image.FromStream(stream);
            stream.Close();
            MakeThumbnailImage(original, newFileName, maxWidth, maxHeight,false);
        }
