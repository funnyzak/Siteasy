...
namespace STA.Data
{
    ...
    public partial class DbHelper
    {
                ...
                if (m_provider == null)
                {
                    lock (lockHelper)
                    {
                        if (m_provider == null)
                        {
                            try
                            {
                                m_provider = (IDbProvider)Activator.CreateInstance(Type.GetType(string.Format("STA.Data.{0}Provider, STA.Data.{0}", BaseConfigs.GetDbType), false, true));
                            }
                            catch
                            {
        ...
        public static void PrepareCommand(DbCommand command, DbConnection connection, DbTransaction transaction, CommandType commandType, string commandText, DbParameter[] commandParameters, out bool mustCloseConnection)
        {
            if (command == null) throw new ArgumentNullException("command");
        ...
        private static DbParameter[] DiscoverSpParameterSet(DbConnection connection, string spName, bool includeReturnValueParameter)
        {
            ...
            connection.ConnectionString = BaseConfigs.GetDBConnectString;
            connection.Open();
        ...
        public static int ExecuteNonQuery(out int id, DbConnection connection, CommandType commandType, string commandText, params DbParameter[] commandParameters)
        {
            ...
            DateTime dt1 = DateTime.Now;
#endif
            id = TypeParse.ObjectToInt(cmd.ExecuteScalar());
#if DEBUG
            DateTime dt2 = DateTime.Now;
        ...
        public static int ExecuteNonQuery(out int id, DbTransaction transaction, CommandType commandType, string commandText, params DbParameter[] commandParameters)
        {
            ...
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = Provider.GetLastIdSql();
            id = TypeParse.ObjectToInt(cmd.ExecuteScalar().ToString());
            return retval;
        }
        ...
        /// (注:主数据库链接即为STA.config文件中Dbconnectstring的链接地址)
        /// </summary>
        /// <remarks>
        /// 示例:  
        ...
        /// <summary>
        /// 在主数据库中执行指定数据库连接对象的数据阅读器.(注:主数据库链接即为STA.config文件中Dbconnectstring的链接地址)
        /// </summary>
        /// <remarks>
        /// 如果是BaseDbHelper打开连接,当连接关闭DataReader也将关闭.
        /// 如果是调用都打开连接,DataReader由调用都管理.
